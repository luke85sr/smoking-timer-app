...
const TIMER_SECONDS_DEFAULT = 40 * 60;
...
async function scheduleEndNotification(delaySec) {
  try {
    console.log('[NOTIFY] Programmo notifica tra secondi:', delaySec);
    await Notifications.scheduleNotificationAsync({
      content: { title: '⏰ Tempo finito!', body: 'Puoi fumare ora.' },
      trigger: { seconds: delaySec, repeats: false },
    });
  } catch (err) {
    console.error('[NOTIFY] Errore scheduling notifica:', err);
  }
}
...
function SettingsScreen({ onSave }) {
  ...
  const handleSave = () => {
    ...
    console.log('[SETTINGS] Salvo dati iniziali:', name, cigs, price);
    onSave({
      name,
...
export default function App() {
  ...
  async function recalcRemaining() {
    try {
      console.log('[TIMER] Ricalcolo tempo residuo...');
  ...
  useEffect(() => {
    (async () => {
      console.log('>>> [INIT] Avvio useEffect principale');
      try {
        const { status } = await Notifications.requestPermissionsAsync();
        console.log('[INIT] Permessi notifiche:', status);

        if (status !== 'granted') {
          RNAlert.alert('Permessi notifiche negati','Abilita le notifiche nelle impostazioni.');
        }

        console.log('[INIT] Creo tabella meta...');
        await runSql(`CREATE TABLE IF NOT EXISTS meta(key TEXT PRIMARY KEY, value TEXT)`);
        console.log('[INIT] Tabella meta creata');
        await runSql(`CREATE TABLE IF NOT EXISTS history(id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT, time TEXT, cost REAL)`);
        console.log('[INIT] Tabella history creata');

        const metaArr = await runSql(`SELECT * FROM meta`);
        console.log('[INIT] metaArr:', metaArr);

        const meta = metaArr.reduce((o, row) => {
          o[row.key] = row.value;
          return o;
        }, {});
        console.log('[INIT] meta:', meta);

        if (meta.appSettings) {
          try {
            setSettings(JSON.parse(meta.appSettings));
            console.log('[INIT] Settings impostati');
          } catch (err) {
            console.error('[INIT] Errore parsing appSettings:', err, meta.appSettings);
          }
        } else {
          console.log('[INIT] meta.appSettings NON trovato');
        }
        ...
        setHistory(hist);
        console.log('[INIT] History caricata:', hist.length);
        ...
        setSummary(sum);
        console.log('[INIT] Summary calcolata:', sum);

        setLoading(false);
        console.log('<<< [INIT] useEffect completata');
      } catch (err) {
        console.error('ERRORE FATALE INIT APP:', err);
  ...
  useEffect(() => {
    if (!settings) return;
    console.log('[SAVE] Salvo impostazioni', settings);
  ...
  useEffect(() => {
    console.log('[SAVE] Stato running:', running);
  ...
  const handleSmoke = () => {
    ...
    console.log('[SMOKE] Inizio nuova sessione. Timer:', totalSec, 'sec');
  };
  ...
  const handleReset = () => {
    RNAlert.alert('Reset','Cancellare tutto?',[
      ...
      { text: 'Sì', style: 'destructive', onPress: async () => {
        ...
        setRunning(false);
        setRemaining(0);
        console.log('[RESET] Tutto cancellato');
      }}
    ]);
  };
  ...
  if (loading) {
    console.log('[UI] Loading, splash screen...');
    return (
  ...
  if (!seenInstructions) {
    console.log('[UI] Mostro istruzioni...');
    return <InstructionsScreen onDone={handleDoneInstructions}/>;
  }
  if (!settings) {
    console.log('[UI] Mostro schermata impostazioni...');
    return <SettingsScreen onSave={setSettings}/>;
  }

  console.log('[UI] Mostro schermata principale');
  return (
    <LinearGradient
